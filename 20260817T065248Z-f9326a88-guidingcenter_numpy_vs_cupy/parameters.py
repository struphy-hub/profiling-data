...
# cupy defaults to device 0, so without this every rank on a node would contend for the
# same GPU instead of getting one each. SLURM_LOCALID (the rank's index within its node)
# is set by srun before this process even starts, so it works without MPI being
